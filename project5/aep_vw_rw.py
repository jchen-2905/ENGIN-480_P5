...
from gwa_parsing.gwa_parser import gwa3_data_parser
import numpy as np
import xarray as xr
...
# === Create constraint with polygon boundaries ===
...
# full_wd = np.arange(0, 360, 1)      
# full_ws = np.arange(3, 25, 1)        

# local_wind = site.local_wind(x_init, y_init, wd=full_wd, ws=full_ws)

# # Extract wind resource arrays
# f = local_wind.Sector_frequency_ilk[0, :, 0]   
# a = local_wind.Weibull_A_ilk[0, :, 0]             
# k = local_wind.Weibull_k_ilk[0, :, 0]             
...
# recorder.save('optimization_vw_rw')
# print('done')
...
# # === Revolution Wind Turbine and Site ===
# class SG_110_200_DD(GenericWindTurbine):
#     def __init__(self):
#         GenericWindTurbine.__init__(self, name='SG 11.0-200 DD', diameter=200, hub_height=140, power_norm=11000, turbulence_intensity=0.08)
...
#         shear=shear)
#         self.initial_position = np.array([rw_xinit, rw_yinit]).T
#         self.name = "Revolutionwind Southforkwind"

# # Turbine types
# turb_types = [0, 1]
# names = ['HaliadeX13MW', 'SG_110_200_DD']
# diameters = [HaliadeX13MW().diameter(), SG_110_200_DD().diameter()]
# hub_heights = [HaliadeX13MW().hub_height(), SG_110_200_DD().hub_height()]
# powerCtFunctions = [HaliadeX13MW().powerCtFunction, SG_110_200_DD().powerCtFunction]
# n_types = len(turb_types)


# # Merge turbine objects
# class CombinedTurbineTypes(WindTurbines):
#     def __init__(self):
#         super().__init__(names, diameters, hub_heights, powerCtFunctions)

# windTurbines = CombinedTurbineTypes()

# rw_wind_turbines = SG_110_200_DD()
# rw_site = Revolutionwind_southforkwind()

# # Define turbine models as a list
# wind_turbines = [vw_wind_turbines, rw_wind_turbines]




'''
# AEP cost model component using turbine_type
aep_model = PyWakeAEPCostModelComponent(
    site=site,
...
boundary_constraint = MultiWFBoundaryConstraint(
    boundaries=[vw_boundary, rw_boundary],
    x=[vw_xinit, rw_xinit],
    y=[vw_yinit, rw_yinit],
    boundary_type=BoundaryType.inside,
    names=['Vineyard Wind', 'Revolution Wind']
)

spacing_constraint = SpacingConstraint(
    min_spacing=4 * 200  # 4 rotor diameters, assuming D=200
)
